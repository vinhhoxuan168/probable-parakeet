 ...
 * All queries in this class structurally match the SQL-05 pattern from
 * styleguide.md Section 5.1:
 * multi-JOIN queries on CustomerReview joined with Product, Customer, and
 * BaseStore.
 */
public class DefaultIS32StoreReviewDao {
    private static final Logger LOG = Logger.getLogger(DefaultIS32StoreReviewDao.class);

    private static final String FIND_REVIEWS_BY_STORE = "SELECT {cr.pk}, {cr.rating}, {p.code}, {c.uid}, {bs.uid} " +
    ...
    private static final String AVG_RATING_BY_STORE_AND_PRODUCT = "SELECT {p.code}, avg({cr.rating}) " +
            "FROM {CustomerReview AS cr " +
            "JOIN Product AS p ON {cr.product} = {p.pk} " +
            "JOIN Customer AS c ON {cr.user} = {c.pk} " +
            "JOIN BaseStore AS bs ON {cr.baseStore} = {bs.pk}} " +
    ...
    private static final String COUNT_REVIEWS_BY_STORE_CUSTOMER = "SELECT {c.uid}, COUNT({cr.pk}) " +
            "FROM {CustomerReview AS cr " +
            "JOIN Product AS p ON {cr.product} = {p.pk} " +
            "JOIN Customer AS c ON {cr.user} = {c.pk} " +
            "JOIN BaseStore AS bs ON {cr.baseStore} = {bs.pk}} " +
